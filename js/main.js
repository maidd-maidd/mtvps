$(function() {
	...
	var calcMargin = function() {
		var margin = parseInt(($('.logo').height()-$('.social').height())/2, 10);	
	...
	*********************/
	...
	if ($('.bg.slideshow').length>0) {
		$('.bg.slideshow').vegas({
			slides:[
				{src:'https://tva1.sinaimg.cn/large/a872541dgy1gp56vm4tqrj21hc0u0hdt.jpg'},
				{src:'https://tvax3.sinaimg.cn/large/a872541dgy1gp56uuwgxsj21hc0zktlc.jpg'},
				{src:'https://tva4.sinaimg.cn/large/a872541dgy1gp56vxgzl0j21hc0u01kx.jpg'},
			],
			transition:'fade',
	...
	**************************/
	if ($("#countdown").length>0) {
		var date = new Date("December 25, 2022 00:00:00");
		...
		if (!mobile) {
			if($(".countdown-section").first().find(".countdown-amount").text().length>2) {
				$(".countdown").css("margin-left", "-30px");
			} else {                
				$(".countdown").css("margin-left", "-44px");
			}
		}
	}
	...
	/*******************
		- Info nav -
	*******************/
	$('.info-nav, .more-info').on("click", function(e) {
		e.preventDefault();

		if ($('.info-nav').hasClass("active")) {			
			$('.bg, .page').toggleClass("modal-opened");			
		} else {
			setTimeout(function() {
				$('.bg, .page').toggleClass("modal-opened");
			}, 500);
	...
	$('#about').on("shown.bs.modal", function() {
		if (!arnica.maps_created) {
			setTimeout(function() {
				googleMaps();
				arnica.maps_created = true;
			}, 500);
		}
	});
